...
sub parse_sql {
    my ($file, $type) = @_;
    my $t = SQL::Translator->new($sqlargs);

    $t->parser($type) or die $t->error;

    my $out = $t->translate("$file") or die $t->error;

    my $schema = $t->schema;

    $schema->name( $file->basename ) unless ( $schema->name );
